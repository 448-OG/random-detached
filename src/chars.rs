...
impl<const N: usize> RandomChars<N> {
    ...
    pub fn generate_chars() -> Result<Zeroizing<[char; N]>, getrandom::Error> {
        let mut seed = [0u8; 32];
        ...
        Self::fill_with_chars(&mut buffer, rng);

        Ok(buffer)
    }

    pub fn generate_chars_with_chacha12() -> Result<Zeroizing<[char; N]>, getrandom::Error> {
        let mut seed = [0u8; 32];
        getrandom::fill(seed.as_mut())?;
